        ...
        {scenes.map((scene) => {
          ...
          return (
            <button
              ...
              className={`
                group w-full text-left text-lg font-medium tracking-tight transition-all duration-300 cursor-pointer
                border-y border-solid
                ${
                  isSelected
                    ? 'text-white border-white/20 pl-5 py-6 md:text-xl' // Highlighted state with subtle top/bottom borders
                    : 'text-[#4a4a4a] border-transparent hover:text-[#7a7a7a] py-4' // Dimmed inactive state
                }
              `}
            >
              {scene}
